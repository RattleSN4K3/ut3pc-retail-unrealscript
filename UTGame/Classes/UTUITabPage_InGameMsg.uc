...
class UTUITabPage_InGameMsg extends UTTabPage_MidGame
	placeable
...
{
	local UTUIScene S;
	local UTUIScene_SendFriendRequest SR;

	S = UTUIScene(GetScene());
	SR = UTUIScene_SendFriendRequest( S.OpenSceneByName(SendFriendRequestScene, false, OnSendFriendRequest_Opened) );
	if ( S != none && SR != none )
	{
		SR.OnSceneDeactivated = SendFriendRequestIsDone;
	    S.FindChild('MidGameSafeRegion',true).PlayUIAnimation('FadeOut',,3.0);
	}
}
