...
function ReadContent()
{
	...
	// Start news read
	NewsInterface.AddReadContentAnnouncementsCompletedDelegate(OnReadContentAnnouncementsCompleted);
	if(NewsInterface.ReadContentAnnouncements(GetPlayerOwner().ControllerId)==false)
	{
...
function OnReadContentAnnouncementsCompleted(bool bWasSuccessful)
{
	...
	NewsInterface.ClearReadContentAnnouncementsCompletedDelegate(OnReadContentAnnouncementsCompleted);

	if(bWasSuccessful)
	{
		// Update content datastore
		StatusLabel.SetDataStoreBinding("");
		ContentDataStore.AvailableContentProvider.ParseContentString(NewsInterface.GetContentAnnouncements(GetPlayerOwner().ControllerId));
		ContentList.RefreshSubscriberValue();
	}
	else
	{
