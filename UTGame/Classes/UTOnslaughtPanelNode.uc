...
simulated event ReplicatedEvent(name VarName)
{
	...
	if (VarName == 'ReplicatedNumPanelsBlownOff')
	{
		if (ReplicatedNumPanelsBlownOff > NumPanelsBlownOff)
		{
			...
			BlowOffRandomPanels(ReplicatedNumPanelsBlownOff - NumPanelsBlownOff, bSpawnExplosion);
...
simulated function BlowOffRandomPanels(int Num, bool bSpawnExplosion)
{
	...
	if (PanelBoneScaler != None && Num > 0)
	{
		PanelMesh.GetBoneNames(BoneNames);
		StartIndex = Rand(BoneNames.length);
		i = StartIndex;
		do
		{
			i++;
			if (i >= BoneNames.length)
			{
				i = 0;
			}
			if (InStr(string(BoneNames[i]), PanelBonePrefix) == 0 && PanelBoneScaler.GetBoneScale(BoneNames[i]) > DESTROYED_PANEL_BONE_SCALE)
			{
				BlowOffPanel(BoneNames[i], bSpawnExplosion);
				Num--;
			}
		} until (Num == 0 || i == StartIndex)
	}
}
...
simulated function BlowOffPanel(name BoneName, bool bSpawnExplosion, optional vector HitLocation)
{
	...
	{
		BoneLocation = PanelMesh.GetBoneLocation(BoneName);
		...
		if (Panel != None)
		{
			...
			{
				Panel.Mesh.AddImpulse(Normal(BoneLocation - PanelMesh.GetPosition()) * 500.0, BoneLocation);
			}
			else
			{
...
simulated function bool BlowOffPanelAtLocation(vector HitLocation, bool bSpawnExplosion)
{
	...
	BoneName = PanelMesh.FindClosestBone(HitLocation,, DESTROYED_PANEL_BONE_SCALE);
	...
	{
		BlowOffPanel(BoneName, bSpawnExplosion, HitLocation);
		return true;
	}
	else
	{
...
simulated function DamagePanels(int Damage, vector HitLocation)
{
	...
	while (PanelHealthRemaining <= 0)
	{
		if (WorldInfo.NetMode != NM_DedicatedServer)
		{
			if (!BlowOffPanelAtLocation(HitLocation, true))
			{
				// if the HitLocation isn't near any panels, just choose one at random
				BlowOffRandomPanels(1, true);
			}
		}
		NumPanelsBlownOff++;
		if (WorldInfo.NetMode != NM_Client)
...
simulated state ObjectiveDestroyed
{
	...
	{
		BlowOffRandomPanels(1 + Rand(3), WorldInfo.TimeSeconds - LastRenderTime < 2.0);
