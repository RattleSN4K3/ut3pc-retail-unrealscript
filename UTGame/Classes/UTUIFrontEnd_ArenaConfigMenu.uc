...
event SceneActivated(bool bInitialActivation)
{
	...
	for (i = 0; i < NumItems; i++)
	{
		...
		if ( class'WorldInfo'.static.IsConsoleBuild() &&
			class'UTUIMenuList'.static.GetCellFieldString(WeaponCombo.ComboList, 'ClassName', i, FieldValue) &&
			Left(FieldValue, 14) ~= "UTGameContent." )
		{
			WeaponCombo.ComboList.RemoveElement(i);
		}
		// skip weapons marked with "NOARENA"
		else if (class'UTUIMenuList'.static.GetCellFieldString(WeaponCombo.ComboList, 'Flags', i, FieldValue))
		{
