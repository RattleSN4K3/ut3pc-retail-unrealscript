...
event PostInitialize()
{
	...
	if ( UIController != None )
	{
		...
		StringListDataStore = UTUIDataStore_StringList(UIController.DataStoreManager.FindDataStore('UTStringList', GetPlayerOwner()));
	}
...
function OnResetToDefaults_Confirm(UTUIScene_MessageBox MessageBox, int SelectionIdx, int PlayerIndex)
{
	...
	if(SelectionIdx==0)
	{
		...
		if(UTScene != None)
		{
			...
			UTUIFrontEnd_Settings(UTScene.GetPreviousScene()).MarkDirty();
		}
	}
}
