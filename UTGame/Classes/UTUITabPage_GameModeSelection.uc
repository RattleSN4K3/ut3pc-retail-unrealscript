...
function OnGameModeChanged(bool bSubmitted)
{
	...
	if(class'UTUIMenuList'.static.GetCellFieldString(GameModeList, 'GameMode', SelectedItem, GameMode) &&
		class'UTUIMenuList'.static.GetCellFieldString(GameModeList, 'DefaultMap', SelectedItem, DefaultMap) &&
		class'UTUIMenuList'.static.GetCellFieldString(GameModeList, 'Prefixes', SelectedItem, Prefixes) &&
		class'UTUIMenuList'.static.GetCellFieldString(GameModeList, 'GameSettingsClass', SelectedItem, GameSettingsClass))
	{
...
function string GetOptionSet()
{
	...
	SelectedItem = GameModeList.GetCurrentItem();
	if(class'UTUIMenuList'.static.GetCellFieldString(GameModeList, 'OptionSet', SelectedItem, OptionSet))
	{
		return OptionSet;
	}
	else
...
function OnGameModeList_ValueChanged( UIObject Sender, optional int PlayerIndex=0 )
{
	...
	if(class'UTUIMenuList'.static.GetCellFieldString(GameModeList, 'PreviewImageMarkup', SelectedItem, StringValue))
	{
		GameModePreviewImage.SetDatastoreBinding(StringValue);
	}

	if(class'UTUIMenuList'.static.GetCellFieldString(GameModeList, 'Description', SelectedItem, StringValue))
	{
