...
function AddMutatorAndFilterList(int NewMutator)
{
	...
	// Group Name
	if (class'UTUIMenuList'.static.GetCellFieldString(EnabledList, 'GroupNames', NewMutator, StringValue))
	{
	...
	if (GroupNames.length > 0)
	{
		...
		for (MutatorIdx = 0; MutatorIdx < EnabledList.Items.length; MutatorIdx++)
		{
			...
			if (class'UTUIMenuList'.static.GetCellFieldString(EnabledList, 'GroupNames', EnabledList.Items[MutatorIdx], StringValue))
			{
