			...
			if (ClassIsChildOf(RequiredPawnClass, class'Vehicle'))
			{
				...
				if (VBase == None)
				{
					...
					for (V = UTGame(WorldInfo.Game).VehicleList; V != None; V = V.NextVehicle)
					{
						...
						{
							if (Vehicle(B.Pawn) != None && (B.MoveTarget == V || (B.RouteCache.length > 1 && B.RouteCache[1] == V.Anchor)))
							{
	...
	event Tick(float DeltaTime)
	{
		...
		while (i < Touchers.length)
		{
			...
			else
			{
				...
				if (PRI != None)
				{
					AddScorer(PRI, (HoldTime == 0.0) ? 1.0 : (DeltaTime / HoldTime));
				}
				i++;
			}
		}
			...
			if (Health < 0)
			{
				if (bReactivateOnUntouch)
				{
					if (bActive)
					{
						...
						// handle re-activating from a separate state
						SetActive(false);
						BroadcastObjectiveMessage(1);
						UTGame(WorldInfo.Game).FindNewObjectives(self);
					}
