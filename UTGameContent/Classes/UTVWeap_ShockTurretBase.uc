...
class UTVWeap_ShockTurretBase extends UTVehicleWeapon
	HideDropDown;

/** This array holds the list of current balls available */
var array<UTProj_VehicleShockBallBase> ShockBalls;
...
function SetComboTarget(UTProj_ShockBall ShockBall)
{
	if (UTBot(Instigator.Controller) != None && Instigator.Controller.Enemy != None)
	{
		...
		ShockBall.Monitor(Instigator.Controller.Enemy);
	}
}

simulated function Projectile ProjectileFire()
{
	local UTProj_VehicleShockBallBase SB;
	local vector VelDir;

	// Add it to the array to track

	SB = UTProj_VehicleShockBallBase(Super.ProjectileFire());
	if (SB!=none)
	{
...
simulated function InstantFire()
{
	...
	local UTProj_ShockBall			BestProj;
...
defaultproperties
{
	...
	DefaultImpactEffect=(ParticleTemplate=ParticleSystem'VH_Hellbender.Effects.P_VH_Hellbender_PrimeImpact',Sound=SoundCue'A_Weapon_ShockRifle.Cue.A_Weapon_SR_AltFireImpactCue')
